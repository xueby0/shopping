...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        //step1:查询购物车
        List<Cart> cartList = cartMapper.findCartListByUserIdAndChecked(userId);
        //step2:List<Cart> --> List<OrderList>
        ServerResponse serverResponse = getCartOrderItem(userId,cartList);
        if (serverResponse.isSuccess()){
            return serverResponse;
        }
    ...
    public ServerResponse send_goods(Long orderNo) {
        ...
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.serverResponseByError("订单未付款");
        }
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://697bev.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("D:/IDEA/workspace/BusinessBackWeb/ftpfile/qr-%s.png",
