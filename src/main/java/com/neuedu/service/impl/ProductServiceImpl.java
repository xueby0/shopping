...
public class ProductServiceImpl implements IProductService {
    ...
    public ServerResponse saveOrUpdate(Product product) {
        ...
        //step2:设置商品的主图 sub_image-->1.jpg,2.jpg,3.jpg
        String subImage = product.getSubImages();
        if (subImage!=null&&!subImage.equals("")){
            String[] subTmageArr=subImage.split(",");
            if (subTmageArr.length>0){
                product.setMainImage(subTmageArr[0]);
            }
        }
        //step3:设置save or update
    ...
    public ServerResponse list_portal(Integer categoryId, String keyword, Integer pageNum, Integer pageSize, String orderBy) {
        ...
        if (orderBy.equals("")){
           ...
           if (orderByArr.length>1){
               PageHelper.startPage(pageNum,pageSize,orderByArr+" "+orderByArr[1]);
           }else{
               PageHelper.startPage(pageNum,pageSize);
           }
        }
